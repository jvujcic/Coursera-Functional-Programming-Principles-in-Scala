...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val occuerrenceMap = y.toMap;
    x.map { case (c, n) => (c, n - occuerrenceMap.getOrElse(c, 0)) }.filterNot(p => p._2 == 0)
  }
      ...
      occurrences match {
        ...
        case _ =>
          for {
            ...
            leftover <- rec(subtract(occurrences, combinations))
          } yield words :: leftover
      }
    }
